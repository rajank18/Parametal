...
export const CategorySelectorModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      ...
      >
        {/* Modal Header */}
        <div className={`px-4 sm:px-6 py-4 sm:py-5 border-b flex items-center justify-between ${isDark ? 'border-zinc-800' : 'border-slate-200'}`}>
          <div>
            <h2 className="text-lg sm:text-xl font-bold">Select Template</h2>
          ...
          <button
            ...
            className={`p-1.5 sm:p-2 rounded-lg border transition-colors ${isDark ? 'border-zinc-800 hover:bg-zinc-900 text-zinc-400' : 'border-slate-200 hover:bg-slate-100 text-slate-500'
              }`}
          >
            <X className="w-4 h-4" />
          </button>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <div
                ...
                className={`group relative p-2.5 sm:p-4 rounded-xl border cursor-pointer transition-all flex flex-col justify-between ${isSelected
                    ? isDark
              ...
              >
                <div>
                  <div
                    className={`w-7 h-7 sm:w-10 sm:h-10 rounded-lg flex items-center justify-center mb-2 sm:mb-3 transition-colors ${isSelected
                  ...
                  <h3 className="text-xs sm:text-sm font-bold group-hover:text-emerald-500 transition-colors line-clamp-1">
                    {cat.name}
                  </h3>
                  <p className={`text-[10px] sm:text-xs mt-0.5 sm:mt-1 leading-tight sm:leading-relaxed ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>
                    {cat.tagline}
                  </p>
                </div>

                <div className="mt-2 sm:mt-4 pt-2 sm:pt-3 border-t border-slate-200/50 dark:border-zinc-800/50 flex items-center justify-between text-[11px] sm:text-xs font-semibold">
                  <span className={isSelected ? 'text-emerald-500 font-bold' : isDark ? 'text-zinc-500' : 'text-slate-400'}>
                    {isSelected ? 'Active' : 'Select'}
                  </span>
                  <ArrowRight className={`w-3 h-3 sm:w-3.5 sm:h-3.5 transition-transform group-hover:translate-x-1 ${isSelected ? 'text-emerald-500' : isDark ? 'text-zinc-500' : 'text-slate-400'
                    }`} />
                </div>
              </div>
            );
        ...
        {/* Modal Footer */}
        <div className={`px-4 sm:px-6 py-3 sm:py-4 border-t flex items-center justify-end ${isDark ? 'border-zinc-800 bg-zinc-900/30' : 'border-slate-200 bg-slate-50'
          }`}>
          <button
            onClick={() => setIsCategoryModalOpen(false)}
            className="px-4 sm:px-5 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold text-xs transition-colors"
          >
            Open Studio
          </button>
        </div>
